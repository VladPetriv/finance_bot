...
// Get provides a mock function with given fields: ctx, balanceID
func (_m *BalanceStore) Get(ctx context.Context, balanceID string) (*models.Balance, error) {
	ret := _m.Called(ctx, balanceID)

	var r0 *models.Balance
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, string) (*models.Balance, error)); ok {
		return rf(ctx, balanceID)
	}
...
type mockConstructorTestingTNewBalanceStore interface {
	mock.TestingT
	Cleanup(func())
}
...
func NewBalanceStore(t mockConstructorTestingTNewBalanceStore) *BalanceStore {
	mock := &BalanceStore{}
	mock.Mock.Test(t)
